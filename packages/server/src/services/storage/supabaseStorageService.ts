...
async function getStorageBucket(): Promise<string> {
  try {
    // Attempt to get the bucket name from Supabase settings table
    ...
    logger.warn(`Error fetching storage bucket config, using default: ${SUPABASE_BUCKET}`, err);
    return SUPABASE_BUCKET;
  }
}
...
function getStorageClient(bucket: string): StorageClientWrapper {
  const storageClient = supabaseClient.getClient().storage.from(bucket);
...
export async function uploadToStorage(
  ...
  try {
    ...
    logger.error(`Error uploading file to Supabase Storage: ${err}`);
    throw err;
  }
}
...
export async function deleteFromStorage(storagePath: string): Promise<void> {
  try {
    ...
    logger.error(`Error deleting file from Supabase Storage: ${err}`);
    throw err;
  }
}
